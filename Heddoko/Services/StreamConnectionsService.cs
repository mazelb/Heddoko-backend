            ...
            if (user.TeamID != null)
            {
                lock (LockObj)
                {
                    ...
                    if (channel == null)
                    {
                        ...
                        BackgroundJob.Enqueue(() => ActivityService.SendForTeam(teamId, UserEventType.StreamChannelOpened, null));
                    }
                }
            }
            ...
            if (user.TeamID != null)
            {
                lock (LockObj)
                {
                    ...
                    if (connections.RemoveAll(c => c.User.Id == user.Id) > 0)
                    {
                        ...
                        int teamId = user.TeamID.Value;
                        BackgroundJob.Enqueue(() => ActivityService.SendForTeam(teamId, UserEventType.StreamChannelClosed, null));
                    }
                }
            }
        }
